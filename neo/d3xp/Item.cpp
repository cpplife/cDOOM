...
================
*/
bool idItem::ModelCallback( renderEntity_t *_renderEntity, const renderView_t *renderView ) {
	const idItem *ent;
	...
	ent = static_cast<idItem *>(gameLocal.entities[_renderEntity->entityNum ]);
	if ( ent == NULL ) {
		gameLocal.Error( "idItem::ModelCallback: callback with NULL game entity" );
		return false;
	}

	return ent->UpdateRenderEntity(_renderEntity, renderView );
}
...
void idItemTeam::SpawnNugget( idVec3 pos ) {
	...
	idPhysics_RigidBody * _physics = static_cast<idPhysics_RigidBody *>( ent->GetPhysics() );

	if (_physics != NULL && _physics->IsType( idPhysics_RigidBody::Type ) ) {
		_physics->DisableImpact();
	}
}
...
void idMoveableItem::Gib( const idVec3 &dir, const char *damageDefName ) {
	...
	if ( *smokeName != '\0' ) {
		const idDeclParticle *_smoke = static_cast<const idDeclParticle *>( declManager->FindType( DECL_PARTICLE, smokeName ) );
		gameLocal.smokeParticles->EmitSmoke( _smoke, gameLocal.time, gameLocal.random.CRandomFloat(), renderEntity.origin, renderEntity.axis, timeGroup /*_D3XP*/ );
	}
	// remove the entity
	PostEventMS( &EV_Remove, 0 );
}
