...
================
*/
void idAFEntity_Base::Event_SetConstraintPosition( const char *_name, const idVec3 &pos ) {
	af.SetConstraintPosition( _name, pos );
}
...
void idAFEntity_VehicleSimple::Think() {
	int i;
	float force = 0.0f, velocity = 0.0f, _steerAngle = 0.0f;
	idVec3 origin;
	idMat3 axis;
	...
	if ( thinkFlags & TH_THINK ) {
		...
		for ( i = 0; i < 2; i++ ) {
			...
			suspension[i]->SetSteerAngle(_steerAngle);
		}
		...
		for ( i = 0; i < 4; i++ ) {
			...
			if ( i < 2 ) {
				...
				steerRotation.SetAngle(_steerAngle);
...
void idAFEntity_VehicleFourWheels::Think() {
	...
	if ( thinkFlags & TH_THINK ) {
		...
		// update the wheel steering
		steering[0]->SetSteerAngle(_steerAngle);
		steering[1]->SetSteerAngle(_steerAngle);
		for ( i = 0; i < 2; i++ ) {
