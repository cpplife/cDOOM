...
====================
*/
void idRenderModelMD5::ParseJoint( idLexer &parser, idMD5Joint *joint, idJointQuat *defaultPose_ ) {
	//
	// parse name
	//
	idToken	token;
	...
	// parse default pose
	//
	parser.Parse1DMatrix( 3, defaultPose_->t.ToFloatPtr() );
	parser.Parse1DMatrix( 3, defaultPose_->q.ToFloatPtr() );
	defaultPose_->q.w = defaultPose_->q.CalcW();
}
...
idRenderModel *idRenderModelMD5::InstantiateDynamicModel( const struct renderEntity_s *ent, const viewDef_t *view, idRenderModel *cachedModel ) {
	...
	if ( staticModel->jointsInverted == NULL ) {
		...
		staticModel->jointsInverted = (idJointMat *)Mem_ClearedAlloc( ALIGN( numInvertedJoints_ * sizeof( idJointMat ), alignment ), TAG_JOINTMAT );
...
====================
*/
jointHandle_t idRenderModelMD5::GetJointHandle( const char *name_ ) const {
	const idMD5Joint *joint = joints.Ptr();
	for ( int i = 0; i < joints.Num(); i++, joint++ ) {
		if ( idStr::Icmp( joint->name.c_str(), name_ ) == 0 ) {
