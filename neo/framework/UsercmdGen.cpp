...
void idUsercmdGenLocal::HandleJoystickAxis( int keyNum, float unclampedValue, float threshold, bool positive ) {
	...
	idGame * game_ = common->Game();
	if ( game_ != NULL ) {
		lookValue *= game_->GetAimAssistSensitivity();
	}
...
void idUsercmdGenLocal::JoystickMove2() {
	...
	idGame * game_ = common->Game();
	const float aimAssist = game_ != NULL ? game_->GetAimAssistSensitivity() : 1.0f;
...
void idUsercmdGenLocal::AimAssist() {
	...
	idGame * game_ = common->Game();
	if ( game_ != NULL ) {
		game_->GetAimAssistAngles( aimAssistAngles );
	}
